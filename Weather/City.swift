...
struct City: CustomStringConvertible {
	...
	init?(from JSON: Dictionary<String, Any>) {
		guard
			...
			let latitude = ((JSON["centroid"] as? Dictionary<String, Any>)?["latitude"] as? String)?.doubleValue,
			let longitude = ((JSON["centroid"] as? Dictionary<String, Any>)?["longitude"] as? String)?.doubleValue,
